class Parser:
    def __init__(self, parsing_char=' '):
    ...
    def parse(self, input_string):
        return input_string.split(self.parsing_char)
...
class CalculatorEngine:
    ...
    def collect_input(self, input_string):
        try:
            ...
            print(f'Error: use identation symbol - "{self._parser.parsing_char}"')
            return {'a': 'No ', 'b': 'result', 'op': '+'}
        return {'a': self.transfer_type(a), 'b': self.transfer_type(b), 'op': operator}
...
class CalculatorInterface:
    ...
    def run(self):
        print(f'Start input(Example: "2 + 2"):')
        while True:
            input_string = input()
